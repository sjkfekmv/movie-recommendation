...
def load_data():
    ...
    return ratings, users, users_all, movies, data, movieid2idx
...
valid_users, movie_matrics, users_matrics, test_data, movieid2idx, users_all, movies = None, None, None, None, None, None, None

def load_all_data():
    global valid_users, movie_matrics, users_matrics, test_data, movieid2idx, users_all, movies
    try:
        print("加载数据...")
        ratings, users, users_all, movies, data, movieid2idx = load_data()
        ...
        for user_id in test_data['UserID'].unique():
            ...
            if len(liked_movies) >= min_liked_movies:
                valid_users.append(user_id)
        valid_users = [int(i) for i in valid_users]
        print(valid_users)
    except Exception as e:
        print(f"Error loading movies data: {e}")

        
# def get_recommend_movie_by_id(user_id):
#     if user_id not in valid_users:
#         raise ValueError(f"无效的用户 ID: {user_id}")
    
#     recommended_indices = recommend_movies_for_user(user_id, test_data, users_matrics, movie_matrics, movieid2idx)
#     user_info = users_all[users_all['UserID'] == user_id].iloc[0]
#     movie_info = movies.iloc[recommended_indices]
...
def get_recommendations():
    user_id = int(request.args.get('user_id'))  # 获取前端传来的 user_id
    print(f"获取推荐电影，用户 ID: {user_id}")
    print(f"有效用户列表: {valid_users}")
    if user_id not in valid_users:
        raise ValueError(f"无效的用户 ID: {user_id}")
    ...
    for _, movie in movie_info.iterrows():
        genres = movie['Genres'].split('|')  # 将电影类别拆分为列表
    ...
    response = {
        "user_info": {
            ...
            "job_id": int(user_info['JobID']),
            "zip_code": user_info['Zip-code']
        },
        "recommended_movies": recommended_movies[:5]  # 只返回前 5 部电影
    }

    return response
    
def test(user_id = 1875):
    recommended_indices = recommend_movies_for_user(user_id, test_data, users_matrics, movie_matrics, movieid2idx)
    user_info = users_all[users_all['UserID'] == user_id].iloc[0]
    movie_info = movies.iloc[recommended_indices]
